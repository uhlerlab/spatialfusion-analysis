...
LOG_FILE="${LOG_DIR}/gcn_fusion_sweep_${TIMESTAMP}.log"
...
/usr/bin/time -v python -X faulthandler "${SCRIPT}" \
  ...
  training.combine_mode=concat,average,gated \
  training.use_cls_loss=true,false \
  training.he_encoder=uni,virchow \
  training.rna_encoder=scgpt,nicheformer \
  training.alignment_mode=full,recon_only \
  training.checkpoint_dir=../../../../SpatialFusion/results/gcn_fusion_sweep/ \
  "$@" 2>&1 | tee "$LOG_FILE"
